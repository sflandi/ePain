<?php
    ...
    $skorFLACC = $query['flacc'];
    $skorCPOT = $query['cpot'];

    if (!empty($id) && $skorFLACC >= '7') {
        include 'tableHeavy.php';
    } elseif (!empty($id) && ($skorFLACC <= '6' && $skorFLACC >='4' )) {
        include 'tableMedium.php';
    }elseif (!empty($id) && ($skorFLACC <= '3' && $skorFLACC >='0' )){
        include 'tableLow.php';
    
    } elseif (!empty($id) && ($skorFLACC == NULL || empty($skorFLACC) )) {
